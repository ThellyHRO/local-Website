 ...
 * 
 * RGBW2-Steuerung eingefügt  * @authors   https://github.com/ThellyHRO/local-Website/graphs/contributors
 * 
 */

/** =============================== CHANGE HERE =============================== */
let ip = '192.168.178.150'; // IP address RGBW2
let REMOTE = {
channel: 2, // Channel for ColorMode: 0 or 0,1,2 or 3 for White-Channel
...
let dimpos = 0;
let CONFIG = {
    bluButtonAddress: "5c:c7:c1:f4:27:fe", // mac in KLEINBUCHSTABEN!
    actions: {
        singlePush: [
            "http://" + ip + "/" + REMOTE.modus + "/" + REMOTE.channel + "?turn=toggle"
        ],
        doublePush: [
            "http://" + ip + "/" + REMOTE.modus + "/" + REMOTE.channel + "?turn=on&" + REMOTE.type + "=100"
        ],
        triplePush: [
            "http://" + ip + "/" + REMOTE.modus + "/" + REMOTE.channel + "?turn=on&" + REMOTE.type + "=10"
        ],
        longPush: [
            "http://" + ip + "/" + REMOTE.modus + "/dimmen"
        ]
    }
};
/** =============================== STOP CHANGING HERE =============================== */
...
function callQueue() {
  if(callsCounter < 6 - urlsPerCall) {
	...
	if(url === "http://" + ip + "/" + REMOTE.modus + "/dimmen") {
	let url = 'http://' + ip + '/' + REMOTE.modus + '/' + REMOTE.channel + '?turn=on&' + REMOTE.type + dimsequence[dimpos];
		dimpos++;
